...
from ..structs import Dependency, DepFileEntry, Outdated
from .utils import compare_versions, get_decoded_file_contents


def _check_dockerfile(file_contents: str) -> list[str]:
    images = []
...
def _check_compose(file_contents: str) -> list[str]:
...
def check(repo: Repository, ref: str, entry: DepFileEntry) -> list[Dependency]:
    print(f" > Checking {entry.path} ...")

    if file_contents := get_decoded_file_contents(repo, ref, entry.path):
        if "Dockerfile" in entry.path:
            images = _check_dockerfile(file_contents)
        else:
            images = _check_compose(file_contents)
        images = list(set(images))
        if len(images):
            return [Dependency(entry=entry, value=image) for image in images]
...
def _get_version_from_tag(tag: str) -> tuple[int, ...]:
    tag = tag.lower()
